...
- [ ] Verify ~400 lines extracted from WorkflowManager
- [ ] Commit: `git add . && git commit -m "feat: extract WorkflowOrchestrator from WorkflowManager"`
...
- [ ] Run tests: `npm test`
- [ ] Ensure no TypeScript errors: `npm run typecheck`
- [ ] Verify ~200 lines extracted from WorkflowManager
- [ ] Commit: `git add . && git commit -m "feat: extract WorkflowPlanningService from WorkflowManager"`
...
- [ ] Run tests: `npm test`
- [ ] Ensure no TypeScript errors: `npm run typecheck`
- [ ] Verify ~150 lines extracted from WorkflowManager
- [ ] Commit: `git add . && git commit -m "feat: extract WorkflowEventCoordinator from WorkflowManager"`
...
- [ ] Ensure no TypeScript errors: `npm run typecheck`
- [ ] Verify ~200 lines extracted from WorkflowManager
- [ ] Commit: `git add . && git commit -m "feat: extract WorkflowStateCoordinator from WorkflowManager"`

## Phase 5: Simplify Constructor with Builder Pattern (Day 5)

### Step 5.1: Create WorkflowManagerBuilder

**File**: `src/core/builders/workflow-manager-builder.ts`
...
**File**: `src/core/__tests__/builders/workflow-manager-builder.test.ts`

```typescript
// NEW FILE - Test builder pattern
import { WorkflowManagerBuilder } from '../../builders/workflow-manager-builder';
...
- [ ] Run tests: `npm test`
- [ ] Ensure no TypeScript errors: `npm run typecheck`
- [ ] Commit: `git add . && git commit -m "feat: add WorkflowManagerBuilder for simplified construction"`
...
- [ ] Run tests: `npm test`
- [ ] Ensure WorkflowManager is ~400-500 lines
- [ ] Verify all extracted services work together
- [ ] Commit: `git add . && git commit -m "feat: create simplified WorkflowManager with extracted services"`
...
### **Key Achievements:**
1. **WorkflowManager reduced from ~1150 lines to ~400 lines** (65% reduction)
2. **Clear separation of concerns** with 5 extracted domain services
3. **Simple constructor** with only 5-7 dependencies (from 12)
4. **Builder pattern** for flexible construction
5. **True DDD principles** with focused domain services

### **Extracted Services:**
- **WorkflowOrchestrator** (~180 lines): Handles all step execution and retry logic
- **WorkflowPlanningService** (~200 lines): Manages planning and replanning
- **WorkflowEventCoordinator** (~140 lines): Coordinates events and monitoring
- **WorkflowStateCoordinator** (~200 lines): Manages state and memory
- **WorkflowManagerBuilder** (~100 lines): Simplifies construction
...
### **Migration Path:**
1. Start by extracting services one at a time
2. Test each extraction thoroughly
3. Use builder pattern for new code
