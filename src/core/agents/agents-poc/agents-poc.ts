...
import { ManagerAgentPrompt, ManagerAgentHumanPrompt } from './agents-poc.prompt';
import { DomService } from '@/infra/services/dom-service';
import {
  DEFAULT_AGENT_MAX_ACTIONS_PER_TASK,
  DEFAULT_AGENT_MAX_RETRIES,
} from './agents-poc.config';
import { ManagerAgentAction, ManagerResponse } from './agents-poc.types';
...
export class AgentsPoc {
  ...
  private summarizer: Agent<unknown>;
  private summarizeTask: AgentTask;
  ...
  constructor(config: AgentsPocConfig) {
  ...
  private onSuccess(result: string) {
    this.reporter.success(`AgentsPoc completed successfully: ${result}`);
    this.isSuccess = true;
    this.result = result;
  ...
  private onFailure(reason: string) {
    this.reporter.failure(`AgentsPoc failed: ${reason}`);
    this.isFailure = true;
    this.reason = reason;
